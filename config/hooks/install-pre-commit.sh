...
# Verify the wrapper's two dependencies exist before installing.
...
if [ ! -f "$REPO_ROOT/tests/run-all.sh" ]; then
  echo "ERROR: test runner missing at tests/run-all.sh" >&2
  exit 1
fi

chmod +x "$HOOK_SRC" "$REPO_ROOT/config/hooks/pre-commit-secret-scan.sh" "$REPO_ROOT/tests/run-all.sh"
